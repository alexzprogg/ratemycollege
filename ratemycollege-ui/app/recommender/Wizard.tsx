...
// ✅ only ONE wizard component exported by default
export default function RateMyCollegeWizard() {
      ...
      <AnimatePresence mode="wait">
        {step === 1 && (
          <motion.div key="step1" {...fadeY}>
            <Section
              ...
              description="Adjust sliders to tell us how important each area is."
            >
                ...
                {PARENTS.map((p) => (
                  <div key={p.id} className="space-y-2">
                    ...
                    <Slider
                      ...
                      max={100}
                      step={1}
                      onValueChange={(v) => onWeightChange(p.id, v)}
                    />
                  </div>
                ))}
              </div>
              ...
              <div className="flex items-center justify-end gap-3 mt-6">
                <Button
                  variant="outline"
                  onClick={() =>
                    setWeights({ social: 50, academics: 50, residence: 50, food: 50, clubs: 50 })
                  }
                >
                  Reset
                </Button>
        ...
        {step === 2 && (
          <motion.div key="step2" {...fadeY} className="space-y-6">
            <Section
              title="Drill down"
              description="Pick a category to refine, then answer a few quick questions."
            >
              <div className="flex flex-wrap gap-2 mb-4">
                {PARENTS.map((p) => (
                  <Button
                    key={p.id}
                    size="sm"
                    variant={chosenParent === p.id ? "default" : "outline"}
                  ...
                  {CHILDREN[chosenParent].map((c) => (
                    <Card key={c.id} className="border-dashed">
                      ...
                      <CardContent className="flex gap-2">
                        <Button
                          size="sm"
                          variant={childAnswers[c.id] === "yes" ? "default" : "secondary"}
                          onClick={() => answerChild(c.id, "yes")}
                        >
                          Yes
                        </Button>
                        <Button
                          size="sm"
                          variant={childAnswers[c.id] === "no" ? "default" : "secondary"}
                          onClick={() => answerChild(c.id, "no")}
                        >
                          No
                        </Button>
                        <Button
                          size="sm"
                          variant={childAnswers[c.id] === "skip" ? "default" : "secondary"}
                          onClick={() => answerChild(c.id, "skip")}
                        >
                          Skip
                        </Button>
                      </CardContent>
          ...
          <motion.div key="step3" {...fadeY}>
            <Section title="Anything else?" description="Optional: tell us specifics and we'll factor it in.">
              <div className="flex items-center gap-2">
                ...
                <Input
                  placeholder="e.g., bright dorms near the library"
                  value={freeText}
              ...
              <div className="flex items-center justify-between mt-6">
                ...
                <div className="flex gap-2">
                  <Button variant="secondary" onClick={() => setFreeText("")}>
                    Skip
                  </Button>
                  <Button onClick={nextStep}>
